              ...
              <div key={section.title} id={section.title.toLowerCase().replace(/[^a-z]+/g, '-').replace(/-$/, '')}>
                ...
                <div className="space-y-2">
                  {section.modules.map((mod) => (
                    <a key={mod.slug} href={'/learn/' + mod.slug} className="flex items-center justify-between p-4 rounded-xl" style={{border: '1px solid #e2e8f0', backgroundColor: '#ffffff', display: 'flex'}}>
                      <div className="flex items-center gap-3">
                        {mod.free ? (
                          <span className="text-xs font-bold px-2 py-1 rounded-full" style={{backgroundColor: '#e8f0fa', color: '#1a5ea5'}}>Free</span>
                        ) : (
                          <span className="text-xs px-2 py-1 rounded-full" style={{backgroundColor: '#f5f4f0', color: '#888780'}}>🔒</span>
                        )}
